...
\begin{titlepage}
  ...
  \horrule{0.5pt} \\[0.4cm] % Thin top horizontal rule
  \Huge Electronic structure with general atomic orbitals on GPGPUs \\
...
Twenty years later, in 1951, Clemens Roothaan used LCAO as a
...
orbital basis sets have formed the backbone of modern computational
quantum chemistry programs. The computational cost of programs based
on atomic orbitals has since been dominated by the integration of
those atomic orbitals. Especially the two-electron terms in the
assembly of the Fock matrix posed a major challenge to the early
development of quantum chemical programs. It was due to the need
for computational efficiency that the computational chemistry
...
poured into their efficient evaluation.
...
such as ADF~\cite{VanLenthe2003, Franchini2013}, adapted Becke's
...
with GTOs.~\cite{Obara1986, Ahlrichs2006}

In recent years the computational landscape has started to
change. Scientific computation has shifted away from personal
computers and towards high-performance computing
clusters.~\cite{Gavini2023} With the
recent rise of general purpose graphical processing units (GPGPU) and
the advent of the exascale computing era, computational codes that
were once regarded as state-of-the-art are dwarfed by other programs that can
...
libraries}~\cite{Lehtola2023c}, in which they argue that open source
...
Schrödinger equation and assume that the nuclei are point like
particles, fixed in space. Their coordinates are given by the set
$\{\mathbf R_1, \cdots ,\mathbf{R}_{N_{\text{nuc}}}\}$.
...
\{\alpha , \beta\}$ by a linear combination of atomic orbitals $\chi$
\begin{equation}
...
\begin{equation}
  V^{\text{nuc}}_{\mu\nu} = -\bra{\mu}
  \sum_{A=1}^{N_{\text{nuc}}} \frac{Z_A}{|\mathbf{r}-\mathbf
  R_A|} \ket{\nu} =\sum_{A=1}^{N_{\text{nuc}}}  -Z_A\int
  \frac{\chi_\mu(\mathbf r) \chi_\nu (\mathbf r)}{|\mathbf r -
  \mathbf R_A|}d\mathbf r
  \label{eq:nuc}
\end{equation}
...
$D^\text{tot}_{\lambda\tau} = D^\alpha_{\lambda\tau} + D^\beta_{\lambda\tau}$.
...
because there does not exist one single basis set of atomic orbitals,
but rather a whole zoo of atomic orbital basis set expansions. In each
...
Normalized Slater Type Orbitals (STO) can be defined as
\begin{equation}
...
$n>l\geq0$ and $l\geq m \geq -l$. $Y_{lm}(\theta,\phi)$ are the real
spherical harmonics and are defined as
\begin{equation}
  Y_{lm}(\theta,\phi) =
...
functions.\cite{Cohen2002,Chong2004, Chong*2005}
...
the same inefficiencies, namely Gaussian Type Orbitals.\cite{Hill2012, Boys1950}

\subsubsection{Gaussian Type Orbitals}\label{subsubsec:gto}
Normalized Gaussian Type Orbitals (GTO) can be defined as
\begin{equation}
...
numbers are $l \geq 0$ and $l\geq m \geq -l$. $Y_{lm}(\theta,\phi)$
are the real spherical harmonics and are defined in equation
\eqref{eq:spherical_harm}.\cite{Hill2012}

GTOs are the most popular atom-centered basis set expansion in the
computational quantum chemistry community,\cite{Hill2012} mainly
because of the Gaussian product theorem.\cite{Boys1950} The Gaussian
product theorem states that the product of two Gaussian functions is
also a Gaussian function, and therefore allows for the analytical
integration of the two-electron integrals   \eqref{eq:coulomb} and
...
basis set is the one of adding more GTOs to the original basis set,
...
Numerical Atom-Centered Orbitals (NAO) can be defined as
\begin{equation}
  \mathcal N_\mu(\mathbf r)= \mathcal N (n,l,m;r,\theta,\phi) =
  \frac{B_{n}(r)}{r}Y_{lm}(\theta,\phi),
\end{equation}
...
the quantum numbers are $n>l\geq0$ and $l\geq m \geq -l$.
$Y_{lm}(\theta,\phi)$ are the real spherical harmonics and are
...
NAO basis sets is usually done by solving a simplified radial ODE
for an individual atom, utilizing numerical methods such as Finite
Element Methods (FEM) and storing the results
numerically.\cite{Blum_2009,Lehtola2019,Lehtola2019a,Lehtola2023b} In
the case of HelFEM the radial part $B_n(r)$ consists of a linear
combination of Lagrange Interpolating Polynomials
(LIP)\cite{Lehtola2019, Lehtola2019a} or Hermite Interpolating
Polynomials (HIP), which can be tabulated and passed on to an
integral evaluation routine.\cite{Lehtola2023b}

In the following sections we will have a closer look at the
computational methods, which allow us to evaluate integrals when
choosing NAOs as our basis set.
...
\begin{equation}
  I = \int_\Omega F(\mathbf r) d\mathbf r \approx
  \sum_{q=1}^{N_{\text{quad}}}F(\mathbf r_q)w_q \quad.
  \label{eq:global_quadrature}
\end{equation}

How the quadrature points and quadrature weights are generated can
...
sub-integral is then evaluated in polar spherical coordinates,
...
\begin{equation}
  I_n = \int F_n(\mathbf r)d^3\mathbf r.
  \label{eq:partial-integral_definition}
\end{equation}

Each partial integral $I_n$ can then be evaluated in spherical polar
coordinates using standard numerical quadrature schemes.

Given a set of $N_\text{nuc}$ spherical quadrature grids, each with
$n_\text{quad}$ points,
$\{\{(\mathbf r_{nq},
w_{nq})\}_{q=1}^{n_{\text{quad}}}\}_{n=1}^{N_{\text{nuc}}}$, where we
denote $(\mathbf r_{nq},w_{nq})$ the $q$-th quadrature point
...
\begin{align}
  ...
  p_n(\mathbf r) &= P_n(\mathbf r) / \sum_m P_m(\mathbf r).
...
though and improvements have been made over Becke's original proposal
of the construction of the relative weight functions.
Laqua et al. introduced a construction that is very close to the
original proposal of Becke, but yields more robust results and has
better screening properties.\cite{Laqua2018}

In their construction $\mu_{ij}$ in equation \eqref{eq:mu_ij} is modified to
...
examine how exactly the quadrature grids are generated.
...
\begin{equation}
  I_n = \iiint F_n(r,\theta,\phi)r^2 \sin \theta drd\theta d\phi.
  \label{eq:I_polar}
\end{equation}
...
\begin{equation}
  I_n = \iint F_n(r,\Omega)r^2  drd\Omega,
  \label{eq:I_solid}
\end{equation}
...
It cannot be overstated that the generation of quadrature grids is an
...
\begin{equation}
  ...
  D^{\text{tot}}_{\lambda\tau}(\mu \nu | \alpha)(\alpha |
  \beta)^{-1}(\beta|\lambda\tau)
  \label{eq:coulomb-RI}
\end{equation}
...
\begin{equation}
  ...
  \beta\lambda\tau}
  D^\sigma_{\lambda\tau}(\mu \lambda | \alpha)(\alpha |
  \beta)^{-1}(\beta|\nu\tau)
...
\begin{align}
  ...
  = \int \chi_\mu(\mathbf{r})\chi_\nu(\mathbf{r}) \tilde V_\alpha(\mathbf{r})
  d\mathbf{r},
...
known analytically for STOs \cite{Cohen2002} and can be efficiently
computed for NAOs.
...
have to be very careful when choosing the framework, which we want to
...
complex tasks the functionalities, which CUDA and HIP provide, are
...
other one will be ignored. Our claim is supported by looking at the
current source code of GauXC, whose CUDA and HIP support has
drastically diverged.  We came to the conclusion that in order to build a
...
debugging tools. The tools are very lightweight and are meant to
...
show later, in section \ref{subsubsec:dense_la} we needed to adapt an
...
matrix is not implemented in Kokkos Kernels at the time of the current
writing.
...
dimensional \texttt{Kokkos::View}s of size $N_\text{nuc}\times
...
\textit{owner} of size  $N_\text{nuc}\times
n_\text{quad}$.

Now that we have fixed the position of the quadrature points, we can move on to
...
\begin{figure}[H]
  \centering
  % --- Horizontal flowchart, full width ---
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
        node distance=8mm,
      ...
      \node[hostbox, right=of dist] (policy)
      {Create one team per\\ quadrature point $(n,q)$};
      \draw[arr] (dist) -- (policy);
      % Team kernel phase
      \node[threadbox, right=12mm of policy] (cache)
      {$r_i = |\mathbf{r}_{nq} - R_i|$};
      \node[threadbox, right=14mm of cache] (weights)
      {$\mu_{ij} = \dfrac{r_i - r_j}{\min(R_c,
        R_{ij})}$\\[1mm]
      $P_i = \prod_{j\neq i} s_k(\mu_{ij})$};
      \node[threadbox, right=14mm of weights] (reduce)
      {$Z = \sum_m P_m(\mathbf{r}_{nq})$};
      \node[teambox, right=of reduce] (write)
      {$p_n = P_n / Z$\\ $w'_{nq} = p_n\, w_{nq}$};
      \draw[arr] (policy) -- (cache);
      \draw[arr] (cache) -- (weights)
      node[midway, above, barrierlabel] {barrier};
      \draw[arr] (weights) -- (reduce)
      node[midway, above, barrierlabel] {barrier};
      \draw[arr] (reduce) -- (write);
      % Dashed team-kernel region
      \begin{scope}[on background layer]
        \node[draw, dashed, rounded corners,
          fit=(cache)(weights)(reduce)(write),
          inner sep=4mm,
          label={[font=\small]below:{Team
        kernel, per $(n,q)$}}] {};
      \end{scope}
  \end{tikzpicture}}
  ...
  {\small
    \begin{tabular}{@{}l l l l l@{}}
      ...
      \texttt{View<Point*>}
      & $N_\text{nuc}\times n_\text{quad}$ & $\mathbf{r}_{nq}$         & --- \\
      \texttt{weights}            &
      \texttt{View<double*>}
      & $N_\text{nuc} \times n_\text{quad}$   & $w_{nq} \to w'_{nq}$
      & --- \\
      \texttt{owner}            &
      \texttt{View<int*>}
      & $N_\text{nuc} \times  n_\text{quad}$    & ---       & --- \\
      \texttt{R\_ij}              &
...
points, used in the remainder of the paper, is therefore
\( N_\text{quad} = n_\text{quad}\times N_\text{nuc} \).
...
the most important implementations. For each term contributing to the
Fock/Kohn-Sham matrix we implemented two different versions. The
first one relies
solely on dense linear algebra kernels and is therefore perfectly suited for
running on GPUs. The second one relies on locally dense linear algebra
...
\subsubsection{Dense Linear Algebra}\label{subsubsec:dense_la}
...
GEneral Matrix Vector (GEMV) multiplications. And indeed when
applying our quadrature rule given by \eqref{eq:global_quadrature} to
...
we use to compute the Coulomb and the exchange matrix are commonly
...
\begin{align}
  ...
  \tilde{V}_\beta(\mathbf r)d\mathbf r \\
...
step, where we remove all eigenvalues below a certain threshold \(
\tau_\text{screen} \) to lower the condition
number of the inverted matrix. The final inverse then reads
...
this work, not implemented in the Kokkos Kernel library, whereas
third-party library calls to LAPACK, CUDA and HIP exist natively in
...
\begin{align}
  ...
  \tilde V_\alpha(\mathbf r) z_\alpha d \mathbf r \\
...
scheme, but we can still take advantage of the rank-k expansion of
...
\begin{align}
  ...
  \tilde V_\alpha(\mathbf r) d \mathbf r \\ 
...
\begin{figure}[H]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \draw[arr] (mia) -- (tik);
      \draw[arr] (tik) -- (kmn);
      \draw[arr] (jmn.east) -- (fock.north west);
      \draw[arr] (kmn.east) -- (fock.south west);
      \draw[arr] (fock.south) to[out=-90, in=-90, looseness=0.5]
      node[midway, below, looplabel] {updated $D_{\mu\nu}$}
      (dens.south);
      ...
      \begin{scope}[on background layer]
        ...
        \node[draw, dashed, rounded corners, draw=gray!60, inner sep=3mm,
          fit=(cig)(mia)(tik),
        label={[looplabel]above:{for each occupied orbital $i$}}] {};
      \end{scope}

      % --- Startup quantities reused each cycle ---
      \draw[arr, dashed] (svd.south) -- (scf.north -| svd.south)
      node[midway, right, looplabel]
      {$\Phi,\ \tilde V,\ X^{-\frac12}$ reused};
  \end{tikzpicture}}

  \caption{Control-flow of the dense RI-J and RI-K integral routines,
    annotated with the underlying equations. The boxes in the top row
    form the one-time startup phase; everything inside the outer dashed
    region is re-evaluated in every SCF cycle. Boxes are colored by
...
\subsubsection{Locally Dense Linear Algebra}\label{subsubsec:locally_dense_la}

The locally dense linear algebra implementation of the RI-J and RI-K
scheme tries to overcome the drawbacks of the dense schemes by not
...
to take advantage of a different feature that atomic orbitals
exhibit, namely their locality.
...
intersection of all discs, generated by taking the origin of a basis
function as the center and the cutoff radius as the radius, with all
bounding boxes. If a disc intersects a bounding box we add it to the
neighbor list.
...
number of points, i.e. a box of size \( B \) contains exactly \( B \)
points. Since we previously ordered our points, we simply define one
box to contain all points with indices \( i \in [(n*B)+1, n*(B+1)]
...
basis-function evaluations \( \phi_i(\mathbf r_g) \) through a fixed-size
neighbor-tile cache in team scratch memory.

Staging the evaluations is what makes the local kernels practical. A
naive local kernel either recomputes each \( \phi_i(\mathbf r_g) \) for
every neighbor pair -- an \( \mathcal O(N_\text{nb}^2\times N_\text{pts})
\) redundancy -- or caches the full \( N_\text{nb}\times N_\text{pts} \)
block, which overflows scratch on large boxes. Tiling resolves both:
each tile of \texttt{tile\_size} neighbors is evaluated once into a
buffer of size \( \texttt{tile\_size}\times\texttt{max\_points\_per\_box}
\) and reused across the paired contraction, so the shared-memory
footprint becomes independent of the number of neighbors. The symmetric
pairwise contraction is carried out as a blocked, lower-triangular tile
loop, halving the arithmetic.
...
=\sum_k C_{ki}\phi_k(\mathbf r_g) \) into the quadrature weights, builds
the half-transformed three-center integral \( (\nu i|\alpha) \) with the
tiled kernel -- evaluating every neighbor tile of \( \phi_j \) once and
...
\begin{figure}[H]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[databox] (dbuild)
      {Build density\\$D_{\mu\nu}=\sum_k f_k C_{\mu k}C_{\nu k}$};
      \node[threadbox, below=16mm of dbuild] (load1)
      {Load $w_g,\mathbf r_g$\\into scratch};
      \node[threadbox, right=of load1] (phi1) {Eval tile\\$\phi_a,\phi_b$};
      \node[threadbox, right=of phi1] (rho)
      {$\rho_g\mathrel{+}=\sum_{ij}\phi_i\phi_j D_{ij}$};
      \node[teambox, right=12mm of rho] (yalpha)
      {$y_\alpha\mathrel{+}=\sum_g \rho_g w_g \tilde V_\alpha(\mathbf r_g)$};
      ...
      {$v_g\mathrel{+}=\sum_\alpha \tilde V_\alpha(\mathbf r_g) w_g z_\alpha$};
      \node[threadbox, right=of vbuild] (phi2) {Eval tile\\$\phi_a,\phi_b$};
      \node[teambox, right=12mm of phi2] (jij)
      {$\tilde J_{ij}\mathrel{+}=\sum_g v_g \phi_i\phi_j$};
      ...
      \draw[arr] (inv) -- (load2) node[midway, right, looplabel] {$z_\alpha$};
  ...
  \caption{Control-flow of the neighbor-tiled RI-J kernel
    ...
    the lower-triangular neighbor-tile loops, in which $\phi_a,\phi_b$
...
\begin{table}[H]
  ...
  {\small
    \begin{tabular}{@{}l l l l l@{}}
      ...
      & $\texttt{tile}\times P_\text{max}$ & $\phi_a$ tile & per team \\
      \texttt{phi\_b}               & \texttt{View<double*, Scratch>}
      & $\texttt{tile}\times P_\text{max}$ & $\phi_b$ tile & per team \\
      \bottomrule
  \end{tabular}}
  \caption{Memory allocations in \texttt{compute\_coulomb\_tiled}. Both
    ...
    $v_g$ for Kernel~2). All device views use \texttt{LayoutLeft};
...
\begin{figure}[H]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[hostbox] (mirror) {Mirror\\$C\to C_h$ (host)};
      \node[hostbox, right=12mm of mirror] (zero) {Zero\\$(\nu i|\alpha)$};
      \node[threadbox, right=12mm of zero] (fold)
      {Load $w_g,\mathbf r_g$;\\$w_g\mathrel{*}=c^i(\mathbf r_g)$};
      \node[threadbox, right=of fold] (phi) {Eval tile\\$\phi_j$};
      \node[threadbox, right=of phi] (pot)
      {$v_\alpha(\mathbf r_g)=\tilde V_\alpha(\mathbf r_g)\, w_g$};
      \node[teambox, right=of pot] (tci)
      {$(\nu i|\alpha)\mathrel{+}=\sum_g v_\alpha(\mathbf r_g)\,\phi_j$};
      \node[hostbox, right=12mm of tci] (gemm1)
      {$T^i=(\nu i|\alpha)^{\!\top}X^{-\frac12}$\\GEMM};
      \node[hostbox, right=of gemm1] (gemm2)
      ...
      \draw[arr] (gemm2.south) to[out=-90, in=-90, looseness=0.3]
      node[midway, below, looplabel] {next orbital $i$} (zero.south);
      \begin{scope}[on background layer]
        \node[draw, dashed, rounded corners, inner sep=11mm,
          fit=(zero)(fold)(phi)(pot)(tci)(gemm1)(gemm2),
        label={[looplabel]above:{for each occupied orbital $i$}}] {};
        \node[draw, dashed, draw=gray!55, rounded corners, inner sep=3mm,
          fit=(fold)(phi)(pot)(tci),
          label={[looplabel, fill=white]above:{one team per box (inner
        loop over aux $\alpha$)}}] {};
      \end{scope}
  \end{tikzpicture}}
  \caption{Control-flow of the neighbor-tiled RI-K kernel
    ...
    weights via $c^i(\mathbf r_g)=\sum_k C_{ki}\phi_k(\mathbf r_g)$, the
    three-center integral $(\nu i|\alpha)$ is built by the tiled team
    kernel (each neighbor tile of $\phi_j$ evaluated once and reused
...
\begin{table}[H]
  ...
  {\small
    \begin{tabular}{@{}l l l l l@{}}
      ...
      & $\texttt{tile}\times P_\text{max}$ & $\phi_j$ tile & per team \\
      \texttt{potential\_scratch}      & \texttt{View<double*, Scratch>}
      & $P_\text{max}$                   & $v_\alpha(\mathbf r_g)$ & per team \\
      \bottomrule
  \end{tabular}}
  \caption{Memory allocations in
    \texttt{compute\_exact\_exchange\_tiled}. All device views use
    \texttt{LayoutLeft}; $P_\text{max}=\texttt{max\_points\_per\_box}$
...
the basis function becomes a massive computational cost. Another
...
be able to evaluate the basis functions during the kernel, hence
the basis functions need to be implemented in our library.

\section{Results}\label{sec:results}

In this section we will present the results of our implementations.
We analyze convergence as well as timing results. We will start with
...
atom, then move on to the Dihydrogen cation and lastly
perform SCF computations on water. Unless an exact value is available in
closed form, the energy errors reported in this section are measured
against a shared self-reference, evaluated on a grid far finer than any
grid in the respective sweep, rather than against an exact or
basis-set-limit value. This isolates the quadrature error from the
...
radial resolution and reaching \( 8.3 \times 10^{-14} \)~Ha. The (M3)
grid is the slowest of the three and still sits at \( 6.3 \times
10^{-12} \)~Ha on the finest grid of the sweep. The two finest (M4)
points barely differ from one another because they have reached the
noise floor of the shared reference, not because the scheme has stopped
converging.
...
n_\mathrm{rad}=40 \) onwards and from there on sits between the (M3) and
...
\begin{figure}[htbp]
  ...
  \caption{Radial-grid convergence of the core-Hamiltonian lowest-MO
    energy of a single hydrogen atom, for three radial quadrature
    schemes: Becke, Treutler-Ahlrichs (M3) ($\alpha=0$) and (M4)
    ($\alpha=0.6$), all with element scaling $\xi(\mathrm{H})=0.8$.
    Basis: QZ4P (ADF Slater-type). Angular grid: Lebedev, fixed
    at order $L=29$. The radial point count is swept over
    $n_\mathrm{rad}\in\{10,20,40,80,160,320,640\}$. The reference is the
    mean of the three schemes on the finest grid ($n_\mathrm{rad}=1280$),
    $E_\mathrm{ref}\approx-0.5$ (a shared self-reference; the
    exact $1s$ energy is $-0.5$~Ha). The ordinate is
  $|E-E_\mathrm{ref}|$.}
...
\begin{figure}[htbp]
  ...
  \caption{Convergence of the numerically integrated overlap integral
    ...
    scheme: Lebedev-Laikov. Left: radial sweep at a fixed angular order
    $L=59$. Right: angular sweep at a fixed radial resolution
...
grows from \( 7.4 \times 10^{-9} \)~Ha at \( L=17 \) to \( 1.7 \times
10^{-8} \)~Ha at \( L=23 \), which we attribute to a partial
...
\begin{figure}[htbp]
  ...
  \caption{Two-dimensional (radial $\times$ angular) grid convergence of
    ...
    $E_\mathrm{ref}=-1.45175161440473$~Ha (shared self-reference; the two
    schemes agree there to $7\times10^{-13}$~Ha). The ordinate is
  $|E-E_\mathrm{ref}|$.}
  \label{fig:conv_2d}
\end{figure}
...
and with Psi4-Robust grid pruning~\cite{Smith2020}. The grid pruning
schemes are implemented in \texttt{IntegratorXX}~\cite{IntegratorXX}.
The total HF-energies are presented in
...
discernible drawback. The Treutler pruning scheme, in contrast, seems
too aggressive in its pruning, as its total error plateaus above \(
10^{-5} \)~Ha: over the last four grid levels it improves by only a
factor of three, despite a sixteen-fold increase in the number of
quadrature points.
...
error. The two one-electron terms, \( E_\mathrm{kin} \) and \(
...
\begin{figure}[h]
  ...
  \caption{Angular pruning-scheme comparison on water (H$_2$O):
    ...
    points), $E_\mathrm{scf}=-76.0667767173$~Ha. Note that the total
...
\begin{figure}[h]
  ...
  \caption{Per-term decomposition of the angular pruning-scheme
    ...
    $E_\mathrm{kin}=75.9976267410$, $E_\mathrm{ne}=-199.0472994426$,
    $E_J=46.7402184304$ and $E_K=-8.9450726577$~Ha. The two
...
the level of a few $\mu$Ha. We now want to benchmark the complete code
on a realistic workload. We ask two questions. The first one being, how
well we can reproduce chemical quantities. The second one being,
...
functional~\cite{Becke93b3lyp} in each of the four ADF Slater-type basis
sets DZ, TZP, TZ2P and QZ4P. The set contains $152$ distinct species,
...
Section~\ref{subsec:grid_construction}, and the dense algorithm. The
SCF is converged to a DIIS error of $10^{-7}$, which we relax to
$10^{-6}$ for open shell species because
their partially filled degenerate shells leave a small residual gradient
that DIIS cannot remove even when the energy is long converged. All
timings were measured on a single GH200 GPU.

A small number of open shell species does not
converge. We exclude every reaction that contains such a species rather
than report an energy the solver never accepted, which leaves $139$,
$131$, $118$ and $121$ of the $140$ reactions for DZ, TZP, TZ2P and
QZ4P respectively. The excluded reactions cluster around a few
individual atoms and radicals, since a single unconverged atom removes
every reaction it takes part in.
...
improves from $52.9$~kcal/mol at DZ to $8.9$ at TZP, $6.2$ at TZ2P and
$5.1$ at QZ4P. The DZ basis carries no polarization functions at all
...
deviation of $-4.0$~kcal/mol rather than at zero. This residual is the
expected behavior of B3LYP itself and not a deficiency of our
integration, since the functional is known to underbind atomization
energies. We take the agreement of this progression with the published
...
\begin{figure}[h]
  ...
  \caption{Distribution of the signed error in the W4-11 total
    ...
    unrestricted B3LYP (Libxc \texttt{hyb\_gga\_xc\_b3lyp3}, the VWN3
    parametrization) with the dense algorithm. The grid is TA-M4 radial
    ...
    clamped-nuclei atomization energies of Ref.~\cite{Karton11w411}.
    Individual reactions are overlaid as jittered points and the
    horizontal bar marks the median. The shaded band is the conventional
    $\pm1$~kcal/mol window of chemical accuracy. The quantity plotted
    is the combined basis set and functional error. The quadrature
    error of Section~\ref{subsec:convergence} is smaller by three to
    four orders of magnitude and is invisible on this scale. The
  summary statistics are given in Table~\ref{tab:w411_summary}.}
...
\begin{table}[h]
  ...
  \caption{Summary of the W4-11 B3LYP atomization energy errors per
    ...
    time per species $\overline{t}$. Systems, method and grid as in
  Figure~\ref{fig:w411_violin}.}
  \label{tab:w411_summary}
  \input{Data/tables/w411_summary}
\end{table}
...
the scaling of the integral kernels themselves. The fitted exponents
rise with the size of the basis, which reflects that the richer sets
place proportionally more of the work in the dense linear algebra steps
that scale most steeply.
...
\begin{figure}[h]
  ...
  \caption{Wall time over the W4-11 set against the number of primary
    ...
    the data. Systems, method and grid as in
  Figure~\ref{fig:w411_violin}.}
...
with about three quarters of the total time. Its absolute cost however
stays nearly constant across the four bases, at roughly $0.7$ to
$1.0$~s per molecule, because it is bound by the size of the grid and
not by the size of the basis. The exact exchange build behaves in the
opposite way. It grows from $0.1$~s per molecule at DZ to $6.9$~s at
QZ4P, where it accounts for $84$ percent of the total runtime and
...
\begin{figure}[h]
  ...
  \caption{Distribution of wall time over the stages of the
    ...
    \emph{SCF} once per Fock build. Systems, method and grid as in
...
properties than the (M3) and (Becke) radial mappings. When performing
Hartree-Fock SCF calculations the error converges to within a few \(
\mu\)Ha, with the one-electron terms posing the accuracy
bottleneck; the two-electron terms converge to sub-\(\mu\)Ha
levels for sufficiently dense quadrature grids.
Regarding pruning schemes we found that the Treutler pruning scheme
plateaus above \( 10^{-5} \)~Ha, whereas the Psi4-Robust pruning
...
deviation from \( 52.9 \) to \( 5.1 \)~kcal/mol and leaves a residual
mean signed deviation of \( -4.0 \)~kcal/mol that reflects the
...
the runtime, but takes up \( 84\% \) of the computational time for
...
convergence properties of the NAOs on polyatomic systems is of great interest.
...
their integration. We expect that adaptive schemes that are directly connected
...
Dr. Susi Lehtola for proposing the thesis project and for their
...
\input{Data/tables/radial_h}

\subsection{H$_2^+$: overlap integral}
\input{Data/tables/h2plus}

\subsection{H$_2^+$: radial $\times$ angular sweep}
\input{Data/tables/2d_becke}
\input{Data/tables/2d_m4}

\subsection{Water: angular pruning schemes}
\input{Data/tables/pruning}

\bibliographystyle{unsrt}
\bibliography{Quadratues.bib}
