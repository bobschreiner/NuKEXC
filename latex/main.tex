...
\begin{titlepage}
  ...
  \Large{Master thesis in Computational Sciences and Engineering
  \\ \textit{submitted by}}
...
\begin{abstract}
  ...
  Atomic orbitals have formed the backbone of computational
  quantum chemistry programs for the past century. While most
  programs rely on Gaussian-type orbitals (GTOs), arguments have been
  made for more general atomic orbital basis sets. The main
  drawback is that numerical quadrature is necessary for the
  integration of basis sets that do not allow for factorization.
  Due to the lack of computational resources, the exploration of
  basis sets has been limited almost exclusively to GTOs.
  The shift of the computational landscape
  towards ever more high-performance computing clusters, and
  the recent popularity of using general-purpose graphics processing
  units (GPGPUs) for scientific computations, have reopened the
  discussion of general atomic orbitals. With this work we provide
  the foundation for a reusable library, capable of harnessing the
  computational power of GPGPUs through the Kokkos ecosystem. The
  library assembles Fock and Kohn-Sham matrices for general atomic
  orbitals. The two-electron terms are evaluated via density fitting,
  and the exchange-correlation terms of the local density
  approximation (LDA) and the generalized gradient approximation (GGA)
  by quadrature on grids built with Becke's fuzzy Voronoi partitioning
  scheme. We show that our quadrature schemes deliver SCF energies
  converged to the \(\mu\)Ha level, and we benchmark the library on
  ...
  functional. We also lay out a roadmap towards larger systems.
...
standard practice of solving this equation is through an
iterative solver, updating the matrix \( \mathbf C^\sigma  \) until
...
available reusable implementations.
...
\begin{equation}
  ...
  R_I|} \ket{\nu} =\sum_{I=1}^{N_{\text{nuc}}}  -Z_I\int
  \frac{\chi_\mu(\mathbf r) \chi_\nu (\mathbf r)}{|\mathbf r -
  \mathbf R_I|}d\mathbf r
  \label{eq:nuc}
\end{equation}
...
\begin{equation}
  I_n = \int F_n(\mathbf r)d\mathbf r.
...
to lower the condition number of the inverted matrix. The final
inverse then reads
    ...
    \begin{tikzpicture}[
      ...
      \begin{scope}[on background layer]
        ...
        \node[draw, dashed, rounded corners, draw=gray!60, inner sep=3mm,
          fit=(cig)(mia)(tik),
        label={[looplabel]right:{for each occupied orbital $i$}}] {};
      \end{scope}
      ...
      node[pos=1, anchor=south east, xshift=-1.5mm, looplabel]
...
\begin{figure}[H]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[threadbox, right=of load1] (phi1) {Eval tile\\$\chi_\mu,\chi_\nu$};
      ...
      \node[threadbox, right=of vbuild] (phi2) {Eval tile\\$\chi_\mu,\chi_\nu$};
...
\begin{figure}[H]
  \centering
  % --- Vertical flowchart: the per-orbital pipeline reads top to bottom, with
...
be able to evaluate the basis functions during the kernel.
...
energies of small molecules, consisting of first and second row
elements. The reference values
...
improves from $52.9$~kcal/mol at DZ to $8.9$~kcal/mol at TZP,
$6.2$~kcal/mol at TZ2P and
$5.1$~kcal/mol at QZ4P. The DZ basis carries no polarization functions at all
...
\begin{figure}[h]
  ...
  \caption{Distribution of the signed error in the W4-11 total
    ...
    horizontal bar marks the median.T he summary statistics are given
