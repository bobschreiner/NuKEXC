...
\begin{equation}
  ...
  \big(\rho(\mathbf r), \gamma(\mathbf r)\big)\, d\mathbf r,
...
the functional on the CPU since there exist no hardware agnostic
library calls to Libxc at the time of writing.
...
The algorithm of Stocks and Barca~\cite{Stocks_2025} is formulated for
the spin-restricted case. Since every benchmark of
Section~\ref{subsec:benchmarks} is unrestricted, we derive the
generalization here.
...
\begin{equation}
  E_\text{XC}^\text{GGA} = \int \varepsilon_\text{XC}^\text{GGA}
  \big( \{\rho^\sigma\}, \{\gamma^{\sigma \sigma'}\} \big)\, d\mathbf r,
  \qquad
...
\begin{figure}[H]
  ...
  \resizebox{!}{0.74\textheight}{%
    \begin{tikzpicture}[
      ...
      \draw[arr, dashed, shorten >=1.5mm] (colloc.east) -| (reusex) -- (zb.east);
...
\begin{table}[H]
  ...
  \caption{Memory allocations in \texttt{compute\_gga\_lsda}. All device
    views use \texttt{LayoutLeft}. The last two rows are allocated inside
...
against the analytic $1s$ energy $E_\mathrm{exact}=-0.5$~Ha and the
...
radial resolution and reaching \( 1.0 \times 10^{-15} \)~Ha. The (M3)
grid is the slowest of the three and still sits at \( 6.4 \times
10^{-12} \)~Ha on the finest grid of the sweep. Because the reference
...
\begin{figure}[htbp]
  ...
  \caption{Radial-grid convergence of the core-Hamiltonian lowest-MO
    ...
    $E_\mathrm{exact}=-0.5$~Ha, so the ordinate
    $|E-E_\mathrm{exact}|$ is pure quadrature error with no
  self-reference noise floor.}
...
$5.1$~kcal/mol at QZ4P. The DZ basis carries no polarization
functions at all
...
\begin{figure}[H]
  ...
  \caption{Distribution of the signed error in the W4-11 total
    ...
    unrestricted B3LYP (\texttt{Libxc}~\cite{Lehtola2018libxc}
      \texttt{hyb\_gga\_xc\_b3lyp3}, the VWN3
...
\begin{figure}[H]
  ...
  \caption{Distribution of wall time over the stages of the
    ...
    \emph{SCF} once per Fock-matrix build. Systems, method and grid as in
...
\input{Data/tables/w411_species}
