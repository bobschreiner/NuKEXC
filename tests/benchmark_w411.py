...
Usage:
    python tests/benchmark_w411.py                      # full sweep, 4 bases
    python tests/benchmark_w411.py --species h2o o h    # just these
    python tests/benchmark_w411.py --bases DZ QZ4P --jobs 8
"""
...
def parse_run(text):
    ...
    # Prefer the solver's fully-converged energy (10 decimals). If the SCF hit
    # its iteration cap, fall back to the last energy the Fock builder printed
    # (same precision) and flag the run.
...
def main():
    ...
    for basis in args.bases:
        ...
        for name in needed:
            ...
            tasks.append((basis, name, basis_dir, charge, mult, thr))

    def execute(task):
        basis, name, basis_dir, charge, mult, thr = task
        jf = cache / ("%s__%s.json" % (basis, name))
        if jf.is_file():
            try:
