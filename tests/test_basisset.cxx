...
TEST_CASE("Shell", "[basisset]") {
  ...
  SECTION("Single Gaussian") {
    ...
    SECTION("S Function") {
      ...
      CHECK(sh.coeff()[0] == Catch::Approx(ncoeff));
      CHECK(sh.size() == 1);
      ...
      CHECK(exact_int == Catch::Approx(1.));
    }
  ...
  SECTION("Multiple Gaussians") {
    ...
    const prim_array coeff = {0.3349460434e-01, 0.2347269535e+00,
                              0.8137573261e+00};
...
TEST_CASE("BasisSet", "[basisset]") {
  ...
  std::vector<int32_t> ref_shell_to_ao = {
      ...
      8,
      11, // O
      (test_spherical ? 16 : 17),
      (test_spherical ? 17 : 18) // H2
  };

  CHECK(basis_map.shell_to_first_ao() == ref_shell_to_ao);
  auto centers_correct = std::none_of(basis_map.shell_to_center().begin(),
                                      basis_map.shell_to_center().end(),
                                      [](auto i) { return i == -1; });
...
/*
 * TODO :: Add back in after integratin hdf5
 *
