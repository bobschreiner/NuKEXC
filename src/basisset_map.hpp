...
class BasisSetMap {
  ...
  using ao_range = std::pair<int, int>;
  using shell_range = std::pair<int, int>;

  int nshells_; ///< Number of basis shells
...
public:
  ...
  BasisSetMap(const GTOBasisSet<F> &basis, const Molecule &mol)
    ...
    for (const auto &shell : basis) {
      auto at_pos = std::find_if(mol.begin(), mol.end(), [&](const Atom &at) {
        return at.x == shell.O()[0] and at.y == shell.O()[1] and
               at.z == shell.O()[2];
      });
      if (at_pos != mol.end())
        shell_to_center_[sh_idx] = std::distance(mol.begin(), at_pos);
      else
        shell_to_center_[sh_idx] = -1;
      ++sh_idx;
    }
  }
   ...
   */
  auto shell_to_first_ao(int i) const { return shell_to_first_ao_.at(i); }
   ...
   */
  auto shell_to_ao_range(int i) const { return shell_to_ao_range_.at(i); }
   ...
   */
  auto shell_size(int i) const { return shell_sizes_.at(i); }
   ...
   */
  const auto &shell_to_center(int i) const { return shell_to_center_[i]; }
   ...
   *  @returns   Basis center index for shell "i"
   */
  auto &shell_to_center(int i) { return shell_to_center_[i]; }
