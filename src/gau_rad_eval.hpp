...
template <typename T>
T gau_rad_eval(int l, int nprim, const T *alpha, const T *coeff, T r) {
  T tmp = 0.;
  const T r2 = r * r;
...
template <typename T>
T gau_rad_cutoff(int l, int nprim, const T *alpha, const T *coeff, T tol) {
  ...
  for (auto i = 0; i < nprim; ++i) {
    ...
    const double prim_cutoff = std::sqrt((log_tol + log_alpha / 2.) / alpha[i]);
    r = std::max(r, prim_cutoff);
  }

  std::vector<T> abs_coeff(coeff, coeff + nprim);
  for (auto &x : abs_coeff)
    x = std::abs(x);
