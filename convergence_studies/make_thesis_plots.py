...
Reads the five convergence_*.csv files from the current working directory and
writes one vector PDF per study. No titles are drawn on any figure (all
provenance goes into the LaTeX caption).

Run it from the build directory holding the CSVs, e.g.

    cd build/convergence_studies
    NUKEXC_FIG_DIR=../../latex/Visualisations \\
        python ../../convergence_studies/make_thesis_plots.py

Without NUKEXC_FIG_DIR the PDFs land next to this script; NUKEXC_CSV_DIR
overrides where the CSVs are read from. (latex/generate_figures.py is the
canonical thesis-figure pass -- it works off the archived copies in
latex/Data/; this script is the quick path straight from a fresh run.)
"""
import csv
import math
import os
